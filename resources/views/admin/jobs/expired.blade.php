...
@section('content')
    <div class="flex justify-between items-center  px-5 py-2 w-full">
        <div>
            <h1 class="text-xl font-bold">Expred  Jobs</h1>
        </div>
        <div>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-black text-white text-left">
                    ...
                    <th class="py-2 px-4 border-b border-l">Posted&nbsp;On</th>
                    <th class="py-2 px-4 border-b border-l">Expired&nbsp;On</th>
            ...
            <tbody>
                @foreach ($jobs as $job)
                    <tr class="{{ $loop->even ? 'bg-gray-300' : 'bg-gray-400' }} px-2 border-l">
                        ...
                        <td class="border-l px-4">{{ $job->deadline }}</td>
