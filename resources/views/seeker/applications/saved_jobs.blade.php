            ...
            @foreach ($jobs as $savedJobs)
                <a href="{{ route('jobs.show', $savedJobs->job->id) }}">
                    <div class="bg-gray-200 p-4 border-2 border-red-600/50 rounded ">
                        <h2 class="text-lg text-left font-semibold">{{ $savedJobs->job->job_title }}</h2>
                        <p class="text-xs italic text-gray-400">
                            ...
                            <span>
                                <i class="fa-solid fa-money-bill"></i> ${{ $savedJobs->job->min_salary }} - ${{ $savedJobs->job->max_salary }}
                            </span>
                        </p>
                        <p class="px-4 py-2 w-full rounded "> {{ $savedJobs->job->name}}</p>
                        <div>
                        <p>{!!Str::limit($savedJobs->job->job_description, 200, '...')!!}</p>
                            <p class="bg-red-600 text-white p-2 rounded my-2 w-3/6 text-center">{{ $savedJobs->job->job_type }}</p>
                        </div>
                    </div>
                </a>
            @endforeach
